...
class TestAuth:
    def test_missing_header_401(self):
        c = _make_client()
        r = c.get("/metrics/reviewer-precision", params=_SINCE)
        assert r.status_code == 401
        assert "invalid API key" in r.json()["detail"]

    def test_wrong_key_401(self):
        c = _make_client()
        r = c.get("/metrics/reviewer-precision", params=_SINCE, headers={"X-API-Key": "nope"})
        assert r.status_code == 401

    def test_empty_configured_key_503(self):
        c = _make_client(api_key="")
        r = c.get("/metrics/reviewer-precision", params=_SINCE, headers={"X-API-Key": "anything"})
...
class TestRequiredSince:
    def test_missing_since_returns_422(self):
        c = _make_client()
        r = c.get("/metrics/reviewer-precision", headers=_AUTH)
        assert r.status_code == 422

    def test_missing_since_on_lead_time_returns_422(self):
        c = _make_client()
        r = c.get("/metrics/lead-time", headers=_AUTH)
        assert r.status_code == 422

    def test_missing_since_on_activity_returns_422(self):
        c = _make_client()
        r = c.get("/metrics/activity", headers=_AUTH)
        assert r.status_code == 422

    def test_missing_since_on_cost_returns_422(self):
        c = _make_client()
        r = c.get("/metrics/cost-by-model", headers=_AUTH)
        assert r.status_code == 422
...
class TestLeadTime:
    def test_returns_rows(self):
        ...
        c = _make_client(rows=rows)
        r = c.get("/metrics/lead-time", params=_SINCE, headers=_AUTH)
        assert r.status_code == 200
...
class TestActivity:
    def test_returns_rows(self):
        rows = [{"author": "dev", "week": datetime(2026, 4, 20, tzinfo=timezone.utc),
                 "prs": 3, "review_runs": 7}]
        c = _make_client(rows=rows)
        r = c.get("/metrics/activity", params=_SINCE, headers=_AUTH)
        assert r.status_code == 200
...
class TestCostByModel:
    def test_returns_rows(self):
        ...
        c = _make_client(rows=rows)
        r = c.get("/metrics/cost-by-model", params=_SINCE, headers=_AUTH)
        assert r.status_code == 200
